...
// Sólo los usuarios con el rol 'notario' pueden editar.

// --- MÓDULO: Estado Global ---
const State = {
    currentTable: 'repertorio_instrumentos_cesar_peinan',
...
const App = {
    ...
    updateTabStyles() {
        ...
        if (State.activeTab === 'registros') {
            if (State.currentTable === 'repertorio_instrumentos_cesar_peinan') {
    ...
    // --- Lógica de Edición ---
    async startEdit(id) {
        const privileged = ['notario', 'notario_suplente', 'supervisor'];
        if (!privileged.includes(window.__userRole)) {
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const handleSession = async (session) => {
            if (typeof isAdmin !== 'undefined' && isAdmin && !session) {
                window.location.href = 'index.html';
                return;
            } 
            
            if (session) {
                // Verify user is active and get role
                const { data: perfil } = await State.supabase
                ...
                window.__userRole = perfil?.rol || 'empleado';
        ...
        // 1. Initial explicit check to prevent blank screen
        const { data: { session: initialSession } } = await State.supabase.auth.getSession();
        await handleSession(initialSession);

        // 2. Listen for future changes
        State.supabase.auth.onAuthStateChange(async (event, session) => {
            // Only handle sign out events here since initial check handled the login
            if (event === 'SIGNED_OUT') {
                window.location.href = 'index.html';
            }
        });
        ...
        if (UI.els['btn-propiedad']) UI.els['btn-propiedad'].addEventListener('click', () => App.switchTable('repertorio_instrumentos_cesar_peinan'));
        ...
        const setupModal = (btnId, dateInputId, title) => {
            ...
            if (btn) {
                ...
                btn.addEventListener('click', () => {
                    if (btnId.includes('inst')) State.currentTable = 'repertorio_instrumentos_cesar_peinan';
        ...
        if (btnIndice) {
            btnIndice.addEventListener('click', () => {
                State.currentTable = 'repertorio_instrumentos_cesar_peinan';
