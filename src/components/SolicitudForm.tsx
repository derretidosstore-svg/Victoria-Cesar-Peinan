...
const inputCls =
  'w-full bg-transparent border-t-0 border-x-0 border-b-2 border-slate-200 py-3 px-0 font-headline text-xl text-slate-900 placeholder:text-slate-300 focus:outline-none focus:border-[#005ab4] transition-colors';

const labelCls =
  'block font-label text-[10px] uppercase tracking-widest text-slate-400 mb-2';
...
function Field({
  ...
  return (
    <div className={`group${full ? ' md:col-span-2' : ''}`}>
...
function AddressField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="md:col-span-2 group" ref={containerRef}>
      ...
      <div className="relative">
        ...
        <span className="absolute right-0 bottom-3 pointer-events-none">
          {loading ? (
            <svg className="h-4 w-4 text-[#005ab4] animate-spin" fill="none" viewBox="0 0 24 24">
          ...
          >
            {suggestions.map((pred) => (
              <li
                ...
                className="flex items-start gap-3 px-5 py-3.5 hover:bg-[#f2f3fd] cursor-pointer transition-colors border-b border-slate-50 last:border-0"
              >
  ...
  if (success) {
    return (
      <div className="p-10 md:p-16 text-center">
        ...
        <h2 className="font-headline text-3xl font-bold text-slate-900 mb-3">¡Solicitud Enviada!</h2>
        <p className="font-body text-slate-500 mb-8 max-w-md mx-auto text-lg leading-relaxed">
          Hemos recibido sus datos correctamente. Haga clic en el siguiente botón para realizar el pago de su trámite.
        </p>
        <button
          className="bg-[#005ab4] text-white font-label text-xs uppercase tracking-widest font-bold px-12 py-5 rounded-xl hover:bg-[#004a96] transition-all active:scale-[0.98] flex items-center gap-3 mx-auto focus:outline-none"
  ...
  return (
    <form onSubmit={handleSubmit} className="p-8 md:p-12 space-y-12">
      ...
      <div className="space-y-8">
        <h2 className="font-headline text-2xl font-bold text-slate-900 border-b border-slate-100 pb-4">
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-[#f2f3fd] border-none py-4 pl-4 pr-10 font-body text-base text-slate-900 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#005ab4]/20 appearance-none transition-all placeholder:text-slate-400"
              placeholder="🔎 Buscar el trámite legal..."
            />
            {isServiceDropdownOpen && (
              <ul className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                {filteredServices.length > 0 ? (
                  filteredServices.map((s: any) => (
                    <li
                      ...
                      className={`px-4 py-3 hover:bg-[#f2f3fd] cursor-pointer transition-colors border-b border-slate-50 last:border-0 ${selectedServiceId === s.id ? 'bg-[#f2f3fd]' : ''}`}
                    >
                      <div className={`font-body text-sm ${selectedServiceId === s.id ? 'font-bold text-[#005ab4]' : 'font-semibold text-slate-800'}`}>
                        {s.titulo}
                      </div>
                      <div className="font-body text-xs text-[#005ab4] font-bold mt-0.5">
        ...
        {selectedService && (
          <div className="bg-[#f2f3fd] border border-[#005ab4]/10 rounded-xl p-7 space-y-5">
            <div className="flex items-center justify-between">
              <h4 className="font-label text-[10px] uppercase tracking-widest text-[#005ab4] font-bold">
                Documentos Requeridos
              </h4>
              <span className="font-label text-xs font-bold text-[#005ab4] bg-[#005ab4]/10 px-3 py-1 rounded-full">
            ...
            {requisitos.length > 0 ? (
              ...
              <p className="font-body text-sm text-slate-500 italic">Sin documentos adicionales requeridos.</p>
            )}
          </div>
        )}
      </div>
      ...
      {/* ── CTA ── */}
      <div className="pt-2 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-t border-slate-100">
        <p className="font-body text-sm text-slate-400 max-w-xs italic leading-relaxed">
          Al continuar, acepta que su información será tratada bajo estricta confidencialidad legal.
        </p>
        <button
          type="submit"
          disabled={loading}
          className={`flex-shrink-0 flex items-center gap-3 font-label text-[11px] uppercase tracking-widest font-bold px-12 py-5 rounded-xl text-white transition-all active:scale-[0.98] focus:outline-none ${
            loading ? 'bg-slate-300 cursor-not-allowed' : 'bg-[#005ab4] hover:bg-[#004a96]'
          }`}
        >
          {loading ? (
            <>
