...
import { updateSolicitud, createSolicitudManual } from '@/app/actions/admin';
import { formatRut } from '@/lib/rut';

export default function SolicitudesManager({ initialSolicitudes, servicios = [] }: { initialSolicitudes: any[], servicios?: any[] }) {
  ...
  const handleStatusChange = async (solicitudId: string, type: 'trabajo' | 'boleta' | 'pago', value: string) => {
    ...
    // Regla de negocio: No se puede finalizar el trámite si la boleta no está emitida
    if (type === 'trabajo' && value === 'listo') {
       const isBoletaEmitida = selectedDoc?.estado_boleta === 'enviada';
       if (!isBoletaEmitida) {
         alert("No se puede marcar el trámite como LISTO si la boleta/factura no ha sido EMITIDA. Para pagos Getnet, la boleta se emite automáticamente al validar el pago.");
         return;
       }
    }
  ...
  const stats = useMemo(() => {
    ...
    initialSolicitudes.forEach(sol => {
      ...
      else if (sol.estado_trabajo === 'en_proceso') enRevision++;
    ...
    return { pendientes, enRevision, completadas };
  ...
  // Estado para mostrar el código generado
  ...
  const handleCreateManual = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.success) {
      ...
      setCreatedCodigo(res.codigo || null);
    } else {
    ...
    <div className="p-8 mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div 
          ...
          className={`cursor-pointer bg-white p-5 rounded-xl shadow-sm border flex flex-col justify-between transition-all hover:scale-[1.01] hover:shadow-md ${filterEstadoTrabajo === 'pendiente' ? 'border-[#005ab4] ring-1 ring-[#005ab4]' : 'border-slate-100'}`}
        >
          <div className="flex justify-between items-start">
            <div className={`p-2 rounded-lg ${filterEstadoTrabajo === 'pendiente' ? 'bg-[#005ab4] text-white' : 'bg-blue-50 text-[#005ab4]'}`}>
            ...
            <span className="text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-100 px-2 py-1 rounded-full uppercase tracking-tighter">Bandeja Entrada</span>
          </div>
          <div className="mt-4">
            <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Nuevas (Pendientes)</p>
            <p className="text-3xl font-bold text-slate-900">{stats.pendientes}</p>
          </div>
        </div>

        <div 
          onClick={() => handleBentoClick('en_proceso')}
          className={`cursor-pointer bg-white p-5 rounded-xl shadow-sm border flex flex-col justify-between transition-all hover:scale-[1.01] hover:shadow-md ${filterEstadoTrabajo === 'en_proceso' ? 'border-amber-500 ring-1 ring-amber-500' : 'border-slate-100'}`}
        >
          <div className="flex justify-between items-start">
            <div className={`p-2 rounded-lg ${filterEstadoTrabajo === 'en_proceso' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-600'}`}>
          ...
          <div className="mt-4">
            <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider">En Revisión / Trabajando</p>
            <p className="text-3xl font-bold text-slate-900">{stats.enRevision}</p>
          </div>
        </div>

        <div 
          onClick={() => handleBentoClick('listo')}
          className={`cursor-pointer bg-white p-5 rounded-xl shadow-sm border flex flex-col justify-between transition-all hover:scale-[1.01] hover:shadow-md ${filterEstadoTrabajo === 'listo' ? 'border-green-500 ring-1 ring-green-500' : 'border-slate-100'}`}
        >
          <div className="flex justify-between items-start">
            <div className={`p-2 rounded-lg ${filterEstadoTrabajo === 'listo' ? 'bg-green-500 text-white' : 'bg-green-50 text-green-600'}`}>
            ...
            <span className="text-[10px] font-bold text-green-600 bg-green-50 border border-green-100 px-2 py-1 rounded-full uppercase tracking-tighter">SOLO HOY</span>
          </div>
          <div className="mt-4">
            <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Trámites Completados</p>
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`w-2.5 h-2.5 rounded-full shadow-sm ${sol.estado_trabajo === 'listo' ? 'bg-emerald-500' : sol.estado_trabajo === 'en_proceso' ? 'bg-blue-500' : 'bg-amber-500'}`}></span>
                        <span className={`font-bold text-[13px] ${sol.estado_trabajo === 'listo' ? 'text-emerald-700' : sol.estado_trabajo === 'en_proceso' ? 'text-blue-700' : 'text-amber-700'}`}>
                          {sol.estado_trabajo === 'en_proceso' ? 'En Revisión' : sol.estado_trabajo.toUpperCase()}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select required name="metodo_pago" className="w-full text-sm sm:text-base bg-slate-50 border-slate-300 rounded-lg py-2.5 px-3 focus:bg-white focus:border-[#005ab4] focus:ring-[#005ab4] transition-colors cursor-pointer">
                    <option value="pos_fisico">🏪 POS Físico (Máquina en Notaría)</option>
                ...
                <div className="bg-amber-50 rounded-xl p-5 border border-amber-200 flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-800 leading-relaxed">
                      Busca los documentos en el correo <strong>tramites@notariavictoria.cl</strong> referenciando el código <span className="font-mono bg-white px-1 py-0.5 rounded text-amber-900 select-all border border-amber-300">#{selectedDoc.codigo || selectedDoc.id.substring(0,8).toUpperCase()}</span>
                    </p>
                  </div>
                </div>
              ...
              <div className="lg:col-span-5 relative">
                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 space-y-6 sticky top-0">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">Método Elegido: <span className="text-slate-700 bg-slate-100 px-2 py-0.5 rounded">{selectedDoc.metodo_pago === 'getnet' ? '💳 Webpay / Getnet' : selectedDoc.metodo_pago === 'pos_fisico' ? '🏪 POS Físico' : selectedDoc.metodo_pago === 'transferencia' ? '🏦 Transferencia' : 'Otro / No indicado'}</span></p>
                    ...
                    {selectedDoc.getnet_session_id && selectedDoc.estado_pago === 'pagado' && (
                      <div className="flex items-center gap-1.5 text-[10px] text-emerald-600 font-bold mt-2 bg-emerald-100 p-2 rounded border border-emerald-200">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
                        </svg>
                        <p>Pago realizado por Webpay/Getnet. Validado automáticamente.</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-end mb-2">
                       ...
                       {(selectedDoc.estado_boleta || 'sin enviar') !== 'enviada' && (
                         <span className="text-[10px] text-amber-600 font-bold bg-amber-50 px-2 py-0.5 rounded border border-amber-100">Falta emitir boleta para finalizar</span>
                       )}
                     </div>
                    <select 
                      ...
                      className={`w-full text-sm font-bold rounded-xl border py-3 px-4 shadow-sm outline-none transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer ${
                        selectedDoc.estado_trabajo === 'listo' ? 'bg-blue-50 border-[#005ab4]/30 text-[#005ab4]' : 'bg-white border-slate-300'
                      }`}
                    >
                      <option value="pendiente">📥 PENDIENTE (Nuevo trámite)</option>
                      <option value="en_proceso">⚙️ EN REVISIÓN (Redacción interna)</option>
                      <option value="listo" disabled={(selectedDoc.estado_boleta || 'sin enviar') !== 'enviada'}>🟢 LISTO (Habilitado para firma/retiro)</option>
                    </select>
                    {(selectedDoc.metodo_pago === 'getnet' || selectedDoc.metodo_pago === 'pos_fisico') && selectedDoc.estado_pago === 'pagado' && (selectedDoc.estado_boleta || 'sin enviar') === 'enviada' && (
                      <p className="text-[10px] text-emerald-600 font-bold mt-2 bg-emerald-100 p-2 rounded border border-emerald-200">
                        ✅ Boleta emitida automáticamente por pago {selectedDoc.metodo_pago === 'getnet' ? 'Getnet' : 'POS Físico'}. Puede marcar como LISTO.
                      </p>
                    )}
                  </div>
      ...
      {createdCodigo && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 text-center animate-in slide-in-from-bottom-8 duration-300">
            <div className="w-16 h-16 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h3 className="text-xl font-bold text-slate-900 mb-2">¡Solicitud Creada!</h3>
            <p className="text-sm text-slate-500 mb-6">Entregue el siguiente código al cliente para que pueda hacer seguimiento de su trámite.</p>
            
            <div className="bg-[#f2f3fd] border-2 border-[#005ab4]/20 rounded-xl p-6 mb-6">
              <p className="text-[10px] uppercase tracking-widest text-[#005ab4] font-bold mb-2">Código de Seguimiento</p>
              <p className="text-4xl font-black font-mono tracking-[0.2em] text-[#005ab4] select-all">{createdCodigo}</p>
            </div>

            <p className="text-xs text-slate-400 mb-6">
              El cliente puede consultar su trámite en <strong>Revisar Solicitud</strong> ingresando este código junto con su RUT.
            </p>

            <button 
              onClick={() => setCreatedCodigo(null)} 
              className="w-full px-5 py-3 rounded-xl font-bold text-white bg-[#005ab4] hover:bg-[#00458a] transition-colors"
            >
              Entendido
            </button>
          </div>
